...
df.counts.summary %>% 
  ggplot(aes(x = gene, y = perc_value)) +
  geom_col() +
...
```{r eval = F}
## DO NOT INCLUDE
reads.n <- c(1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000)

df.percentage.coverage <- df.counts %>% 
  group_by(sequence_id) %>% 
  mutate(
    total_n_genes = n(),
    reads_n = list(reads.n)
  ) %>% 
  unnest(reads_n) %>% 
  mutate(
    min_reads = counts >= reads_n
  ) %>% 
  group_by(sequence_id, reads_n) %>% 
  summarise(
    min_n_reads = sum(min_reads) / unique(total_n_genes)
  )

df.percentage.coverage %>% 
ggplot(aes(x = reads_n, y = min_n_reads, group = sequence_id)) +
  geom_point() +
  geom_line() +
  facet_wrap(~sequence_id) +
  scale_x_log10() +
...
df.norm <- df.counts.f %>% 
  group_by(gene) %>% 
  mutate(
    pseudo_reference = mean(log(counts))
  ) %>% 
  group_by(sequence_id) %>% 
  mutate(
    ratio = ifelse(is.finite(pseudo_reference), log(counts) - pseudo_reference, NA),
    size_factor = exp(median(ratio, na.rm = T)),
    counts_norm = counts / size_factor
  ) %>% 
  ungroup()
```
...
# LEAVE OUT
df.norm.pca <- df.norm %>% 
  select(gene, sequence_id, counts_norm) %>%
  f_pca(counts_from = "counts_norm", features_from = "gene", samples_from = "sequence_id")
```
