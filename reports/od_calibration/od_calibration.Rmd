...
df.od.calibration.models %>%
  filter(multicultivator == 0, channel == 1) %>%
  predict_tidy_linear_model_range( od_mc_raw, from = 0, to = 2, .interval = 0.5)
```
...
df.od.calibration.corrected <- left_join(
  ...
  mutate( predictions = map(model, predict_tidy_linear_model_range, x = od_mc_raw, from = 0, to=1, .interval = 0.01) )
```
